...
### (a) Linear model predicting `mpg` from `wt` and `hp`
...
### (c) Multicollinearity using `car::vif()`
...
### (d) Add interaction between `wt` and `hp`
...
### (a) Boxplot comparing `V1` between benign and malignant tumors
...
### (b) Logistic regression predicting malignancy from V1, V2, and V3
...
### (c) Predicted probabilities, confusion matrix, accuracy, sensitivity, and specificity
...
### (d) Predicted probability for a new observation
...
## (b) Fit and compare three models
...
## (c) Fixed effects and ICC for participants
...
## (d) Random slopes for condition by participant
...
## (e) Plot individual participant regression lines
...
# Design choice:
# I added a cylinder filter and a checkbox for the regression line.
# The cylinder filter lets users explore whether the relationship changes across car types,
# while the checkbox makes it easier to compare the raw scatter plot with the fitted trend.

shinyApp(ui = ui, server = server)
```

This app lets the user choose a predictor variable from `wt`, `hp`, `disp`, and `drat`, then fits a linear model predicting `mpg` from that selected predictor. I added a cylinder filter and a regression-line toggle as enhancements. These features make the app more useful for exploration because the user can examine whether the relationship changes across subsets of cars and whether the trend line helps summarize the pattern.

# Part 5: Parallel Computing

## 5.1 System Setup and Benchmarking

### (a) Number of cores and safe core choice
...
### (b) Sequential vs. parallel benchmark
...
### (a) Sequential bootstrap confidence interval
...
### (b) Parallel bootstrap confidence interval
...
### (c) Compare bootstrap sample sizes
